using System;
using TripsEngineService;

namespace HotelReservationEngine.DataParser
{
    public class BookingParser
    {
        public CompleteBookingRQ GetPricing()
        {
            return new CompleteBookingRQ()
            {
                ResultRequested = TripsEngineService.ResponseType.Unknown,
                SessionId = Guid.NewGuid().ToString(),
                ExternalPayment = new Payment()
                {
                    Amount = new TripsEngineService.Money()
                    {
                        BaseEquivAmount = 0,
                    ...
                    Attributes = new StateBag[]
                {
                    ...
                    new StateBag()
                    {
                        Name="ProvideAuthorizationTransactionId",
                    },
                    new StateBag()
                    {
                ...
                //CreditCardMake creditCardMake  = new CreditCardMake()
                //{
                //    Code = "VI",
                //    Name = "Visa",
                //};
